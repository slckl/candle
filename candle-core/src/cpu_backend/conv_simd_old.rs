                ...
                (0..p.c_out).into_par_iter().for_each(|dst_c_idx| {
                    ...
                    for b_idx in 0..p.b_size {
                        ...
                        for dst_h in 0..out_h {
                            ...
                            // SIMD-optimized convolution inner loop for avx2.
                            ...
                            // Fallback for non-x86_64 architectures or without AVX2.
...
fn simd_conv_inner<T: WithDType>(
    ...
    if std::any::TypeId::of::<T>() == std::any::TypeId::of::<f32>() {
        ...
        simd_conv_inner_generic::<T>(
            inp_cont, k_cont, dst, h_dst_idx, h_src_idx, cont_s2, out_w, offset_w, p,
        );
    }
}
...
fn simd_conv_inner_f32(
    ...
    // Process 8 output positions in parallel using AVX
    const LANES: usize = 8;
        ...
        for chunk in 0..num_chunks {
            ...
            for lane in 0..LANES {
                ...
                if src_w < p.padding || src_w >= p.i_w + p.padding {
                    ...
                    src_offsets[lane] = h_src_idx + (src_w - p.padding) * cont_s2;
                }
            }

            // Process channel dimension with SIMD for all valid lanes
            let c_chunks = p.c_in / 8;
            ...
            for c_idx in (c_chunks * 8)..p.c_in {
    ...
    for dst_w in (num_chunks * LANES)..out_w {
        ...
        if src_w >= p.padding && src_w < p.i_w + p.padding {
            ...
            let mut d = 0.0f32;
            unsafe {
                crate::cpu::vec_dot_f32(inp_slice.as_ptr(), k_cont.as_ptr(), &mut d, p.c_in);
                let ptr = dst.as_ptr().add(dst_idx) as *mut f32;
                *ptr += d;
            }
        }
...
fn simd_conv_inner_f64(
    ...
    unsafe {
        for chunk in 0..num_chunks {
            ...
            for c_idx in (c_chunks * 4)..p.c_in {
                let k_val = *k_cont.get_unchecked(c_idx);
                for lane in 0..LANES {
                    if valid_mask[lane] {
                        let inp_val = *inp_cont.get_unchecked(src_offsets[lane] + c_idx);
...
fn simd_conv_inner_generic<T: WithDType>(
    ...
    for dst_w in 0..out_w {
        ...
        let mut d = T::zero();
        unsafe {
            T::vec_dot(inp_slice.as_ptr(), k_cont.as_ptr(), &mut d, p.c_in);
